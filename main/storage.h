...
esp_err_t storage_init(void);       /* open NVS, migrate version; call after nvs_flash_init() */
esp_err_t storage_erase_all(void);  /* erase carradio namespace, reset to phase 0 */
...
/* ── Stream URL (legacy — superseded by playlist_json) ───────────────── */
...
bool      storage_is_phase_ready(void);   /* true if playlist_json + WiFi SSID exist */
...
esp_err_t storage_get_bt_mac(char *buf, size_t len);       /* "AA:BB:CC:DD:EE:FF" */
esp_err_t storage_set_bt_mac(const char *mac_str);

esp_err_t storage_get_bt_mac_bytes(uint8_t mac[6]);        /* parses stored MAC string */
...
esp_err_t storage_get_tm_enabled(uint8_t *out);   /* default 1 */
esp_err_t storage_set_tm_enabled(bool enabled);

/* ── Validation helpers ──────────────────────────────────────────────── */

bool storage_validate_ssid(const char *ssid);   /* non-empty, <= 32 chars */
bool storage_validate_url(const char *url);     /* starts with http:// or https:// */
bool storage_validate_mac(const char *mac_str); /* "AA:BB:CC:DD:EE:FF" */
